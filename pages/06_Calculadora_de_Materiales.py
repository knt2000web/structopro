...
# ── Imports opcionales — ReportLab (PDF) ─────────────────────────────────────
try:
  ...
  from reportlab.platypus import (SimpleDocTemplate, Table as RLTable,
                   ...
                   Paragraph as RLParagraph, Spacer as RLSpacer,
...
# ── Imports opcionales — Matplotlib (gráficas en DOCX) ───────────────────────
try:
  import matplotlib
  matplotlib.use("Agg")
...
# ── FACTORES DE DESPERDICIO POR DEFECTO (editables por proyecto) ──────────────
DESPERDICIO_DEFAULT = {
  "cemento":   0.05,
  "arena":    0.12,
  "grava":    0.12,
...
def get_desperdicio(material_key):
  """Retorna factor de desperdicio configurado (0.0-1.0)."""
  df_d = st.session_state.get("kc_cfg_desperdicios")
  if df_d is not None and not df_d.empty:
    try:
      row = df_d[df_d["Material"] == material_key]
      if not row.empty:
        return float(row.iloc[0]["Desperdicio (%)"]) / 100.0
    except Exception:
      pass
  return DESPERDICIO_DEFAULT.get(material_key, 0.05)

# ── HUELLA DE CARBONO — FACTORES DE EMISIÓN (kg CO₂ por unidad) ───────────────
CO2_FACTORES = {
  "cemento_bolsa":  {"Colombia": 25.0, "Perú": 24.5, "México": 25.5,
            "Ecuador": 24.5, "USA": 22.0, "default": 25.0},
  "acero_kg":    0.89,
  "arena_m3":    2.5,
  "grava_m3":    3.0,
  "bloque_und":   1.2,
...
def _get_co2_factor(material, pais):
  f = CO2_FACTORES.get(material)
  if isinstance(f, dict):
    return f.get(pais, f.get("default", 25.0))
  return f or 0.0

def _pu_mat(material_key, fallback, con_iva=False):
  """Precio unitario desde session_state con fallback hardcodeado."""
  val = st.session_state.get("kc_precios", {}).get(material_key, fallback)
  try:
    val = float(val)
  except (TypeError, ValueError):
    val = float(fallback)
  if con_iva:
    _norma = st.session_state.get("norma_sel", "")
    _iva_factor = 1.19 if _norma.startswith("NSR") else 1.0
    val = val * _iva_factor
  return val
...
_PERSIST_KEYS = [
  "kc_rows", "empresa", "proyecto", "ingeniero",
...
def save_state():
  """Persiste las claves críticas del session_state en disco (JSON).
  logo_bytes se serializa como base64 para compatibilidad JSON.
  """
  try:
    ...
    for k in _PERSIST_KEYS:
      ...
      if k == "logo_bytes" and isinstance(val, (bytes, bytearray)):
        snapshot[k] = {"__type__": "bytes_b64", "data": _b64.b64encode(val).decode()}
      elif val is not None:
        # Serializar solo tipos seguros
        try:
          json.dumps(val)
          snapshot[k] = val
        except (TypeError, ValueError):
    ...
    pass  # No crashear si el disco no está disponible

def load_state():
  """Carga el estado persistido. Deserializa logo_bytes desde base64."""
  if os.path.exists(STATE_FILE):
    try:
      import base64 as _b64
      with open(STATE_FILE, 'r', encoding='utf-8') as f:
...
# ── HELPER: Gráficos Plotly en modo claro uniforme ────────────────────────────
def _plotly_light(fig, height=360):
  """Aplica tema claro a cualquier figura Plotly."""
  fig.update_layout(
    height=height,
    plot_bgcolor="#ffffff",
...
st.set_page_config(page_title="Calculadora de Materiales", layout="wide", page_icon="")
...
def get_live_prices(pais, ref_prices):
  """Intenta obtener precios en vivo por scraping.
  Actualiza session_state["_price_status_label"] y ["_price_date"]
  para que el PDF y el DOCX reflejen siempre el estado real de los precios.
  """
  urls = PRICE_URLS.get(pais, {})
  result = dict(ref_prices)
  _actualizado = []   # materiales que sí se actualizaron en vivo
  ...
  if "cemento" in urls:
    ...
    if vals:
      result["cemento"] = sum(vals) / len(vals)
      _actualizado.append("cemento")
  ...
  if "teja_barro" in urls:
    ...
    if vals:
      ...
      _actualizado.append("teja_barro")
  ...
  if "ceramica" in urls:
    ...
    if vals:
      ...
      _actualizado.append("cerámica")

  # ── Registrar estado de frescura de precios ──────────────────────────────
  _fecha_ahora = datetime.datetime.now().strftime("%d/%m/%Y %H:%M")
  st.session_state["_price_date"] = _fecha_ahora
  if _actualizado:
    st.session_state["_price_status"] = "live"
    st.session_state["_price_status_label"] = (
      f"Precios en vivo actualizados: {', '.join(_actualizado)} — {_fecha_ahora}"
    )
  else:
    st.session_state["_price_status"] = "fallback"
    st.session_state["_price_status_label"] = (
      f"Scraping sin datos — usando precios de referencia ({_fecha_ahora}). "
      "Verifique con cotizaciones actualizadas."
    )
...
# ══════════════════════════════════════════════════════════════════════════════
# HELPER: MODO CLARO OBLIGATORIO PARA FIGURAS EN DOCX
# ══════════════════════════════════════════════════════════════════════════════
def fig_to_docx_white(fig):
  """Convierte una figura matplotlib a PNG modo claro sin alterar la pantalla.
  Retorna bytes PNG con fondo blanco (#ffffff), texto #1a1a1a, ejes #cccccc.
  """
  import matplotlib.pyplot as plt
  import matplotlib
  import copy
  ...
  for ax, backup in zip(fig.get_axes(), axes_backup):
    ax.set_facecolor(backup["fc"])
    ax.title.set_color(backup["title"])
    ax.xaxis.label.set_color(backup["xlabel"])
    ax.yaxis.label.set_color(backup["ylabel"])
    for sname, scol in backup["spines"].items():
      ax.spines[sname].set_edgecolor(scol)

  return png_bytes


# ══════════════════════════════════════════════════════════════════════════════
# HELPER: MEMORIA DE CUANTIFICACIÓN — DOCX
# ══════════════════════════════════════════════════════════════════════════════
def build_docx_materiales(rows, precios, R, logo_bytes=None):
  """Genera la Memoria de Cuantificación de Materiales en DOCX.
  8 secciones: Portada (con logo), Parámetros, Materiales, Agrupación,
  Gráfica de Costos, Verificaciones de precios, Responsables, Referencias.
  """
  if not _DOCX_OK:
    return None
  ...
  def _fila(tbl, vals, bold=False, bg=None):
    row = tbl.add_row()
  ...
  _logo_bytes_doc = logo_bytes or st.session_state.get("logo_bytes")
  if _logo_bytes_doc:
    try:
      from docx.shared import Inches
      _logo_buf = BytesIO(_logo_bytes_doc)
      _logo_para = doc.add_paragraph()
      _logo_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
      _logo_run = _logo_para.add_run()
      _logo_run.add_picture(_logo_buf, width=Cm(5.0))
      doc.add_paragraph()
    except Exception:
      pass  # Si la imagen falla, continuar sin ella

  t_portada = doc.add_paragraph()
  t_portada.alignment = WD_ALIGN_PARAGRAPH.CENTER
  run = t_portada.add_run(st.session_state.get("empresa","________________") or "________________")
  run.font.size = Pt(20); run.font.bold = True; run.font.color.rgb = _AZUL

  doc.add_paragraph()
  t2 = doc.add_paragraph()
  t2.alignment = WD_ALIGN_PARAGRAPH.CENTER
  run2 = t2.add_run("MEMORIA DE CUANTIFICACIÓN DE MATERIALES")
  run2.font.size = Pt(16); run2.font.bold = True

  doc.add_paragraph()
  for lbl, val in [
    ("Proyecto:", st.session_state.get("proyecto","________________") or "________________"),
    ("Norma:",   R.get("_norma","---")),
    ("País:",   R.get("pais","---")),
    ("Moneda:",  R.get("moneda","---")),
    ...
    r_lbl = p.add_run(lbl + " ")
    r_lbl.font.bold = True; r_lbl.font.color.rgb = _AZUL
    p.add_run(val)

  doc.add_page_break()

  # ═══════════════════ CAP 1 — PARÁMETROS ══════════════════════════════════
  _heading("1. Parámetros y Precios de Referencia")
  doc.add_paragraph(f"Normativa aplicada: {R.get('_norma','---')} | País: {R.get('pais','---')} | Moneda: {R.get('moneda','---')}")

  tbl1 = doc.add_table(rows=1, cols=3)
  tbl1.style = "Table Grid"
  _tabla_hdr(tbl1, ["Material", "Precio unitario", "Unidad"])
  PRICE_LABELS = {
    "cemento": ("Cemento / bolsa", "bolsa"),
    "arena":  ("Arena", "m³"),
    "grava":  ("Grava / Gravilla", "m³"),
    "acero_kg":("Acero de refuerzo", "kg"),
    "bloque": ("Bloque / CMU", "und"),
    "ladrillo":("Ladrillo", "und"),
    "pintura": ("Pintura", "galón"),
    "ceramica":("Cerámica", "m²"),
  }
  for k, (lbl, und) in PRICE_LABELS.items():
    v = precios.get(k)
    if v is not None:
      _fila(tbl1, [lbl, f"{R.get('moneda','$')} {float(v):,.2f}", und])

  doc.add_paragraph()

  # ═══════════════════ CAP 2 — CUANTIFICACIÓN ═══════════════════════════════
  _heading("2. Cuantificación de Materiales")

  if rows:
    df = pd.DataFrame(rows)
    df["subtotal"] = df["cant"].astype(float) * df["precio"].astype(float)
    total_global  = df["subtotal"].sum()

    tbl2 = doc.add_table(rows=1, cols=5)
    tbl2.style = "Table Grid"
    _tabla_hdr(tbl2, ["Elemento / Material", "Unidad", "Cantidad",
              f"P.U. ({R.get('moneda','$')})", f"Total ({R.get('moneda','$')})"])
    for _, row in df.iterrows():
      _fila(tbl2, [
        row.get("elemento",""),
        row.get("unidad",""),
        f"{float(row.get('cant',0)):,.3f}",
        f"{float(row.get('precio',0)):,.2f}",
        f"{float(row.get('subtotal',0)):,.2f}",
      ])
    _fila(tbl2, ["", "", "", "TOTAL", f"{total_global:,.2f}"], bold=True, bg="D6E4F0")
  else:
    doc.add_paragraph("No hay materiales registrados en el resumen.")

  doc.add_paragraph()

  # ═══════════════════ CAP 3 — AGRUPACIÓN POR CATEGORÍA ════════════════════
  _heading("3. Resumen por Categoría")
  if rows:
    df_grp = df.groupby("elemento")["subtotal"].sum().reset_index()
    df_grp = df_grp.sort_values("subtotal", ascending=False)
    tbl3  = doc.add_table(rows=1, cols=3)
    tbl3.style = "Table Grid"
    _tabla_hdr(tbl3, ["Categoría", f"Total ({R.get('moneda','$')})", "% del total"])
    for _, row in df_grp.iterrows():
      pct = (row["subtotal"] / total_global * 100) if total_global else 0
      _fila(tbl3, [row["elemento"], f"{row['subtotal']:,.2f}", f"{pct:.1f}%"])

    # ── GRÁFICA DE DISTRIBUCIÓN DE COSTOS (MODO CLARO) ────────────────────
    if _MPL_OK and len(df_grp) > 0:
      try:
        import matplotlib.pyplot as _plt2
        _fig_dc, (_ax_bar, _ax_pie) = _plt2.subplots(1, 2, figsize=(10, 4))

        # Barras horizontales
        _colores_bar = ["#1565c0","#1976d2","#2196f3","#42a5f5","#90caf9",
                        "#e53935","#ef5350","#ff7043","#ffa726","#4caf50"]
        _cats  = df_grp["elemento"].tolist()[:10]
        _vals  = df_grp["subtotal"].tolist()[:10]
        _n     = len(_cats)
        _cols_b = _colores_bar[:_n]
        _bars  = _ax_bar.barh(_cats, _vals, color=_cols_b)
        _ax_bar.set_xlabel(f"Total ({R.get('moneda','$')})", fontsize=9)
        _ax_bar.set_title("Distribución de Costos", fontsize=11, fontweight="bold")
        _ax_bar.tick_params(axis="y", labelsize=8)
        _ax_bar.tick_params(axis="x", labelsize=8)
        for _bar in _bars:
          _w = _bar.get_width()
          _ax_bar.text(_w * 1.01, _bar.get_y() + _bar.get_height() / 2,
                       f"{R.get('moneda','$')} {_w:,.0f}", va="center", fontsize=7)

        # Torta
        _ax_pie.pie(
          _vals,
          labels=[c[:18] for c in _cats],
          colors=_cols_b,
          autopct="%1.1f%%",
          startangle=140,
          pctdistance=0.82,
        ...
        _ax_pie.set_title("Participación %", fontsize=11, fontweight="bold")
        _fig_dc.tight_layout()

        # Convertir a PNG modo claro y pegar en el doc
        _png_dc  = fig_to_docx_white(_fig_dc)
        _plt2.close(_fig_dc)
        _img_buf = BytesIO(_png_dc)
        doc.add_paragraph()
        _img_para = doc.add_paragraph()
        _img_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
        _img_run  = _img_para.add_run()
        _img_run.add_picture(_img_buf, width=Cm(15.0))
        _cap = doc.add_paragraph("Figura 1 — Distribución de costos por categoría de material")
        _cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
        for _r in _cap.runs:
          _r.font.size = Pt(8)
          _r.font.italic = True
          _r.font.color.rgb = _GRIS
      except Exception:
        pass  # Si matplotlib falla, continúa sin gráfica

  doc.add_paragraph()

  # ═══════════════════ CAP 4 — VERIFICACIONES DE PRECIOS ═══════════════════
  _heading("4. Verificación de Precios")
  doc.add_paragraph(
    "Los precios unitarios reflejados en este documento corresponden a precios de referencia "
    f"del mercado de {R.get('pais','---')} al {datetime.datetime.now().strftime('%d/%m/%Y')}. "
    "Deben verificarse contra cotizaciones actualizadas antes de la contratación formal."
  )
  tbl4 = doc.add_table(rows=1, cols=3)
  tbl4.style = "Table Grid"
  _tabla_hdr(tbl4, ["Parámetro", "Fuente", "Estado"])
  _fila(tbl4, ["Precios materiales", "PRICE_URLS / cotización manual", "Referencial"])
  _fila(tbl4, ["Desperdicios aplicados", "Ver tab de cálculo específico", "Incluido"])
  _fila(tbl4, ["IVA / AIU", f"Normativa {R.get('pais','---')}", "Incluido según APU"])

  doc.add_paragraph()

  # ═══════════════════ CAP 5 — IDENTIDAD DEL DOCUMENTO ══════════════════════
  _heading("5. Responsables")
  tbl5 = doc.add_table(rows=1, cols=2)
  tbl5.style = "Table Grid"
  _tabla_hdr(tbl5, ["Rol", "Nombre"])
  for rol, key in [
    ...
    _fila(tbl5, [rol, st.session_state.get(key,"________________") or "________________"])
  doc.add_paragraph()

  # ═══════════════════ CAP 6 — REFERENCIAS ══════════════════════════════════
  _heading("6. Referencias Normativas")
  refs = [
    f"• {R.get('_norma','---')} — Norma de diseño aplicable",
    "• ICONTEC 2289 — Planos y documentos de ingeniería (cuando aplique)",
    "• Decreto de salarios mínimos vigente del país correspondiente",
    "• Precios de referencia: ver PRICE_URLS en código fuente",
  ]
  for ref in refs:
...
# ══════════════════════════════════════════════════════════════════════════════
# HELPER: PLANILLA DE MATERIALES — DXF (ICONTEC 2289)
# ══════════════════════════════════════════════════════════════════════════════

# ══════════════════════════════════════════════════════════════════════════════
# EXPORTAR — Memoria de Cuantificación en PDF (ReportLab)
# ══════════════════════════════════════════════════════════════════════════════
def build_pdf_materiales(rows, precios, R, logo_bytes=None):
  """Genera la Memoria de Cuantificación en PDF. Requiere reportlab."""
  if not _PDF_OK:
    return None
  buf = BytesIO()
  doc = SimpleDocTemplate(
    ...
    title="Memoria de Cuantificación"
  )
  styles = getSampleStyleSheet()
  AZUL   = rl_colors.HexColor("#1e3a5f")
  AZUL_CL  = rl_colors.HexColor("#d6e4f0")
  GRIS   = rl_colors.HexColor("#444444")
  sT = RLPStyle("tit", parent=styles["Title"],
    textColor=AZUL, fontSize=18, spaceAfter=6, alignment=RL_CENTER)
  sS = RLPStyle("sub", parent=styles["Normal"],
    textColor=GRIS, fontSize=11, spaceAfter=4, alignment=RL_CENTER)
  sH = RLPStyle("h2",  parent=styles["Heading2"],
    textColor=AZUL, fontSize=13, spaceBefore=14, spaceAfter=6)
  sB = RLPStyle("body", parent=styles["Normal"],
  ...
  if logo_bytes:
    try:
      story.append(RLImage(BytesIO(logo_bytes), width=4*rl_cm, height=2*rl_cm))
      story.append(RLSpacer(1, 0.3*rl_cm))
    except Exception:
      pass

  _emp = st.session_state.get("empresa", "________________") or "________________"
  _proy = st.session_state.get("proyecto", "________________") or "________________"
  _ing = st.session_state.get("ingeniero","________________") or "________________"
  _fecha = datetime.datetime.now().strftime("%d/%m/%Y")
  moneda_sym = R.get("moneda","$")

  story.append(RLParagraph(_emp, sT))
  story.append(RLParagraph("MEMORIA DE CUANTIFICACIÓN DE MATERIALES", sS))
  story.append(HRFlowable(width="100%", thickness=1.5, color=AZUL, spaceAfter=8))

  meta = [["Proyecto:", _proy],["Norma:", R.get("_norma","---")],
      ["País:", R.get("pais","---")],["Moneda:", moneda_sym],
      ["Ingeniero:", _ing],["Fecha:", _fecha]]
  mt = RLTable(meta, colWidths=[3.5*rl_cm, 12*rl_cm])
  mt.setStyle(RLTableStyle([
    ("FONTSIZE",(0,0),(-1,-1),9),
  ...
  story.append(mt)
  story.append(RLSpacer(1, 0.8*rl_cm))
  story.append(HRFlowable(width="100%", thickness=0.5, color=rl_colors.lightgrey))
  story.append(RLSpacer(1, 0.4*rl_cm))

  # 1. Precios de referencia
  story.append(RLParagraph("1. Precios de Referencia", sH))
  PL = {"cemento":("Cemento/bolsa","bolsa"),"arena":("Arena","m³"),
     "grava":("Grava","m³"),"acero_kg":("Acero refuerzo","kg"),
     "bloque":("Bloque/CMU","und"),"ladrillo":("Ladrillo","und"),
     "pintura":("Pintura","galón"),"ceramica":("Cerámica","m²")}
  pr_rows=[["Material",f"Precio ({moneda_sym})","Unidad"]]
  for k,(lbl,und) in PL.items():
    v=precios.get(k)
    if v is not None:
      pr_rows.append([lbl, f"{float(v):,.2f}", und])
  pt=RLTable(pr_rows, colWidths=[7*rl_cm,4*rl_cm,3*rl_cm])
  pt.setStyle(RLTableStyle([
    ("BACKGROUND",(0,0),(-1,0),AZUL),("TEXTCOLOR",(0,0),(-1,0),rl_colors.white),
    ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),8),
    ("ROWBACKGROUNDS",(0,1),(-1,-1),[rl_colors.white, AZUL_CL]),
    ("GRID",(0,0),(-1,-1),0.4,rl_colors.lightgrey),("BOTTOMPADDING",(0,0),(-1,-1),4),
  ]))
  story.append(pt)
  story.append(RLSpacer(1, 0.5*rl_cm))

  # 2. Cuantificación
  story.append(RLParagraph("2. Cuantificación de Materiales", sH))
  if rows:
    import pandas as _pd2
    _df = _pd2.DataFrame(rows)
    _df["subtotal"] = _df["cant"].astype(float) * _df["precio"].astype(float)
    total_g = _df["subtotal"].sum()
    td=[["Elemento / Material","Unidad","Cantidad",
       f"P.U. ({moneda_sym})",f"Total ({moneda_sym})"]]
    for _, row in _df.iterrows():
      td.append([str(row.get("elemento","")),str(row.get("unidad","")),
            f"{float(row.get('cant',0)):,.3f}",
            f"{float(row.get('precio',0)):,.2f}",
            f"{float(row.get('subtotal',0)):,.2f}"])
    td.append(["","","","TOTAL",f"{total_g:,.2f}"])
    ct=RLTable(td, colWidths=[6*rl_cm,2*rl_cm,2.5*rl_cm,3*rl_cm,3*rl_cm])
    ct.setStyle(RLTableStyle([
      ("BACKGROUND",(0,0),(-1,0),AZUL),("TEXTCOLOR",(0,0),(-1,0),rl_colors.white),
      ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),
      ("FONTNAME",(0,-1),(-1,-1),"Helvetica-Bold"),
      ("BACKGROUND",(0,-1),(-1,-1),AZUL_CL),
      ("FONTSIZE",(0,0),(-1,-1),8),
      ("ROWBACKGROUNDS",(0,1),(-1,-2),[rl_colors.white,rl_colors.HexColor("#f5f5f5")]),
      ("GRID",(0,0),(-1,-1),0.4,rl_colors.lightgrey),
      ("ALIGN",(2,1),(-1,-1),"RIGHT"),("BOTTOMPADDING",(0,0),(-1,-1),4),
    ]))
    story.append(ct)
    story.append(RLSpacer(1, 0.5*rl_cm))

    # 3. Resumen por categoría
    story.append(RLParagraph("3. Resumen por Categoría", sH))
    _grp = _df.groupby("elemento")["subtotal"].sum().reset_index()
    _grp = _grp.sort_values("subtotal", ascending=False)
    gd=[["Categoría",f"Total ({moneda_sym})","% del total"]]
    for _, row in _grp.iterrows():
      pct=(row["subtotal"]/total_g*100) if total_g else 0
      gd.append([row["elemento"],f"{row['subtotal']:,.2f}",f"{pct:.1f}%"])
    gt=RLTable(gd, colWidths=[8*rl_cm,4*rl_cm,3*rl_cm])
    gt.setStyle(RLTableStyle([
      ("BACKGROUND",(0,0),(-1,0),AZUL),("TEXTCOLOR",(0,0),(-1,0),rl_colors.white),
      ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),8),
      ("ROWBACKGROUNDS",(0,1),(-1,-1),[rl_colors.white, AZUL_CL]),
      ("GRID",(0,0),(-1,-1),0.4,rl_colors.lightgrey),
      ("ALIGN",(1,1),(-1,-1),"RIGHT"),("BOTTOMPADDING",(0,0),(-1,-1),4),
    ]))
    story.append(gt)
  else:
    story.append(RLParagraph("No hay materiales registrados.", sB))

  story.append(RLSpacer(1, 0.5*rl_cm))

  # 4. Estado de precios
  story.append(RLParagraph("4. Verificación de Precios", sH))
  _ps = st.session_state.get("_price_status_label","Precios de referencia (no verificados en vivo)")
  _pd_date = st.session_state.get("_price_date","N/D")
  story.append(RLParagraph(
    f"Estado de precios: <b>{_ps}</b> — Última consulta: <b>{_pd_date}</b><br/>"
    "Los precios deben verificarse con cotizaciones actualizadas antes de contratación formal.",
    sB))

  story.append(RLSpacer(1, 0.5*rl_cm))
  # 5. Responsables
  story.append(RLParagraph("5. Firmas y Responsables", sH))
  rd=[["Rol","Nombre"]]
  for rol,key in [("Empresa/Firma","empresa"),("Ingeniero Responsable","ingeniero"),
          ("Elaboró","dxf_elaboro"),("Revisó","dxf_reviso"),("Aprobó","dxf_aprobo")]:
    rd.append([rol, st.session_state.get(key,"________________") or "________________"])
  rt=RLTable(rd, colWidths=[5*rl_cm,10*rl_cm])
  rt.setStyle(RLTableStyle([
    ("BACKGROUND",(0,0),(-1,0),AZUL),("TEXTCOLOR",(0,0),(-1,0),rl_colors.white),
    ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),8),
    ("GRID",(0,0),(-1,-1),0.4,rl_colors.lightgrey),("BOTTOMPADDING",(0,0),(-1,-1),4),
  ]))
  ...
  # 6. Notas normativas
  story.append(RLParagraph("6. Referencias Normativas", sH))
  story.append(RLParagraph(f"• {R.get('_norma','---')} — Norma de diseño aplicable", sB))
  story.append(RLParagraph("• ICONTEC 2289 — Planos y documentos de ingeniería (cuando aplique)", sB))
  story.append(RLParagraph("• Precios incluyen factores de desperdicio configurados por proyecto.", sB))

  doc.build(story)
  buf.seek(0)
  return buf

def build_dxf_planilla(rows, precios, R):
  """Genera planilla de materiales en DXF con rótulo ICONTEC 2289.
  Capas: CONTORNO 0.50mm | TEXTO 0.18mm | ROTULO 0.35mm | TABLA 0.25mm
  Todo en Color 7 (blanco/negro). PROHIBIDO amarillo/cian/verde.
  """
  if not _DXF_OK:
    return None
  ...
  # ── Capas ICONTEC ─────────────────────────────────────────────────────────
  def _capa(nombre, lw):
    layer = doc.layers.new(name=nombre)
    layer.color = 7
    layer.lineweight = lw
    return nombre

  L_CONTORNO = _capa("CONTORNO", 50)
  L_TEXTO  = _capa("TEXTO",  18)
  L_ROTULO  = _capa("ROTULO",  35)
  L_TABLA  = _capa("TABLA",  25)

  # ── Helpers de texto ─────────────────────────────────────────────────────
  def _txt(x, y, texto, h=3.5, layer=L_TEXTO, just="LEFT"):
    attribs = {"height": h, "layer": layer, "color": 7}
    if just == "CENTER":
      attribs["halign"] = 4 # ezdxf MIDDLE_CENTER alignment via TEXT
      msp.add_text(str(texto), dxfattribs={**attribs, "insert": (x, y), "align_point": (x, y), "halign": 4, "valign": 0})
    else:
      msp.add_text(str(texto), dxfattribs={**attribs, "insert": (x, y)})

  def _box(x1, y1, x2, y2, layer=L_TABLA):
    msp.add_line((x1, y1), (x2, y1), dxfattribs={"layer": layer, "color": 7})
    msp.add_line((x2, y1), (x2, y2), dxfattribs={"layer": layer, "color": 7})
    msp.add_line((x2, y2), (x1, y2), dxfattribs={"layer": layer, "color": 7})
    msp.add_line((x1, y2), (x1, y1), dxfattribs={"layer": layer, "color": 7})

  def _hline(x1, x2, y, layer=L_TABLA):
    msp.add_line((x1, y), (x2, y), dxfattribs={"layer": layer, "color": 7})

  def _vline(x, y1, y2, layer=L_TABLA):
    msp.add_line((x, y1), (x, y2), dxfattribs={"layer": layer, "color": 7})

  # ── Dimensiones hoja A3 horizontal ───────────────────────────────────────
  W, H = 420.0, 297.0
  M  = 10.0  # margen

  # Borde de hoja
  _box(M, M, W - M, H - M, layer=L_CONTORNO)

  # ── RÓTULO ICONTEC (esquina inferior derecha) ─────────────────────────────
  rx1, ry1 = W - M - 180, M
  rx2, ry2 = W - M,    M + 55

  _box(rx1, ry1, rx2, ry2, layer=L_ROTULO)

  # Divisiones del rótulo
  row_h = (ry2 - ry1) / 4
  for i in range(1, 4):
    _hline(rx1, rx2, ry1 + i * row_h, layer=L_ROTULO)

  mid_x = rx1 + (rx2 - rx1) / 2
  _vline(mid_x, ry1, ry2, layer=L_ROTULO)
  ...
  _proy  = st.session_state.get("proyecto","________________")  or "________________"
  _ing  = st.session_state.get("ingeniero","________________") or "________________"
  _elab  = st.session_state.get("dxf_elaboro","___")      or "___"
  _rev  = st.session_state.get("dxf_reviso","___")       or "___"
  _apb  = st.session_state.get("dxf_aprobo","___")       or "___"
  _fecha = datetime.datetime.now().strftime("%d/%m/%Y")

  _txt(rx1 + pad, ry2 - row_h * 0.6, _emp, h=4.5, layer=L_ROTULO)
  _txt(rx1 + pad, ry2 - row_h * 1.6, _proy, h=3.5, layer=L_ROTULO)
  _txt(rx1 + pad, ry2 - row_h * 2.4, "Ing.:", h=3.0, layer=L_ROTULO)
  _txt(rx1 + 15, ry2 - row_h * 2.4, _ing, h=3.0, layer=L_ROTULO)

  # Bloque elaboró/revisó/aprobó/fecha
  _txt(rx1 + pad, ry1 + pad + 2, "Elaboró:", h=2.5, layer=L_ROTULO)
  _txt(rx1 + 22,  ry1 + pad + 2, _elab,    h=2.5, layer=L_ROTULO)
  _txt(mid_x + pad, ry1 + pad + 2, "Revisó:",  h=2.5, layer=L_ROTULO)
  _txt(mid_x + 18, ry1 + pad + 2, _rev,    h=2.5, layer=L_ROTULO)
  _txt(rx1 + pad, ry1 + row_h + pad + 2, "Aprobó:", h=2.5, layer=L_ROTULO)
  _txt(rx1 + 22,  ry1 + row_h + pad + 2, _apb, h=2.5, layer=L_ROTULO)
  _txt(mid_x + pad, ry1 + row_h + pad + 2, "Fecha:", h=2.5, layer=L_ROTULO)
  _txt(mid_x + 16, ry1 + row_h + pad + 2, _fecha, h=2.5, layer=L_ROTULO)

  # Norma
  _txt(rx1 + pad, ry1 + 3 * row_h + pad + 1, R.get("_norma","---"), h=2.5, layer=L_ROTULO)

  # ── TÍTULO PLANILLA ───────────────────────────────────────────────────────
  titulo = f"PLANILLA DE MATERIALES — {R.get('pais','---')} | {_proy}"
  _txt(M + 5, H - M - 8, titulo, h=5.0, layer=L_TEXTO)
  _hline(M, W - M - 180, H - M - 11, layer=L_CONTORNO)

  # ── TABLA DE MATERIALES ───────────────────────────────────────────────────
  if rows:
    # Columnas: Elemento, Unidad, Cantidad, P.U., Total
    col_x = [M + 5, M + 80, M + 110, M + 140, M + 165, M + 195]
    col_headers = ["Elemento / Material", "Und.", "Cantidad",
            f"P.U. {R.get('moneda','$')}", f"Total {R.get('moneda','$')}"]
    ...
    # Cabecera
    _hline(tbl_left, tbl_right, tbl_top,     layer=L_TABLA)
    _hline(tbl_left, tbl_right, tbl_top - row_h_t, layer=L_TABLA)
    for cx in col_x:
      _vline(cx, tbl_top - row_h_t, tbl_top, layer=L_TABLA)
    _vline(tbl_right, tbl_top - row_h_t, tbl_top, layer=L_TABLA)
    for i, hdr in enumerate(col_headers):
      _txt(col_x[i] + 1, tbl_top - row_h_t + 2, hdr[:22], h=2.8, layer=L_ROTULO)

    # Filas de datos
    df_dxf = pd.DataFrame(rows)
    df_dxf["subtotal"] = df_dxf["cant"].astype(float) * df_dxf["precio"].astype(float)
    total_dxf = df_dxf["subtotal"].sum()

    cur_y = tbl_top - row_h_t
    for _, row in df_dxf.iterrows():
      y_bot = cur_y - row_h_t
      _hline(tbl_left, tbl_right, y_bot, layer=L_TABLA)
      for cx in col_x:
        _vline(cx, y_bot, cur_y, layer=L_TABLA)
      _vline(tbl_right, y_bot, cur_y, layer=L_TABLA)
      vals = [
        str(row.get("elemento",""))[:28],
        str(row.get("unidad","")),
        f"{float(row.get('cant',0)):,.3f}",
        f"{float(row.get('precio',0)):,.0f}",
        f"{float(row.get('subtotal',0)):,.0f}",
      ]
      for i, v in enumerate(vals):
        _txt(col_x[i] + 1, y_bot + 2, v, h=2.5)
      cur_y = y_bot
      if cur_y < ry2 + 20:
        break # evitar solapar rótulo
    ...
    for cx in col_x:
      _vline(cx, y_bot, cur_y, layer=L_TABLA)
    _vline(tbl_right, y_bot, cur_y, layer=L_TABLA)
    _txt(col_x[0] + 1, y_bot + 2, "TOTAL", h=3.0, layer=L_ROTULO)
    _txt(col_x[4] + 1, y_bot + 2, f"{total_dxf:,.0f}", h=3.0, layer=L_ROTULO)

    # ── TABLA DE PRECIOS DE REFERENCIA ────────────────────────────────────
    pt_top  = min(cur_y - row_h_t - 15, H - M - 80)
    pt_left = tbl_left
    pt_right = M + 120
    pt_row  = 6.5

    _txt(pt_left, pt_top + 5, "PRECIOS DE REFERENCIA", h=3.5, layer=L_ROTULO)
    pr_headers = ["Material", f"Precio ({R.get('moneda','$')})", "Und."]
    pr_cx = [pt_left, pt_left + 60, pt_left + 95, pt_right]
    _hline(pt_left, pt_right, pt_top,    layer=L_TABLA)
    _hline(pt_left, pt_right, pt_top - pt_row, layer=L_TABLA)
    for cx in pr_cx:
      _vline(cx, pt_top - pt_row, pt_top, layer=L_TABLA)
    for i, hdr in enumerate(pr_headers):
      _txt(pr_cx[i] + 1, pt_top - pt_row + 2, hdr, h=2.5, layer=L_ROTULO)

    PRICE_UND = {"cemento": "bolsa", "arena": "m³", "grava": "m³",
           "acero_kg": "kg", "bloque": "und", "ladrillo": "und",
           "pintura": "galón", "ceramica": "m²"}
    cur_py = pt_top - pt_row
    for mat, und in PRICE_UND.items():
      v = precios.get(mat)
      if v is None: continue
      y_b = cur_py - pt_row
      _hline(pt_left, pt_right, y_b, layer=L_TABLA)
      for cx in pr_cx:
        _vline(cx, y_b, cur_py, layer=L_TABLA)
      _txt(pr_cx[0] + 1, y_b + 2, mat,     h=2.3)
      _txt(pr_cx[1] + 1, y_b + 2, f"{float(v):,.0f}", h=2.3)
      _txt(pr_cx[2] + 1, y_b + 2, und,      h=2.3)
      cur_py = y_b
      if cur_py < ry2 + 15: break
...
def build_excel_presupuesto(df_pres, cliente, proyecto):
  ...
  # Fill header — usar session_state de identidad
  ws['B5'] = f"Nombre: {cliente or st.session_state.get('ingeniero','') or '________________'}"
  ws['B8'] = f"Proyecto: {proyecto or st.session_state.get('proyecto','') or '________________'}"
  ws['D4'] = st.session_state.get('empresa','') or '________________'
...
# ── MARCA BLANCA — Campos de identidad obligatorios ───────────────────────────
for _ik in ["empresa","proyecto","ingeniero","dxf_elaboro","dxf_reviso","dxf_aprobo"]:
  if _ik not in st.session_state:
    st.session_state[_ik] = ""

# 
# HEADER
# 
_header_empresa = st.session_state.get("empresa","") or "________________"
_header_proyecto = st.session_state.get("proyecto","") or "________________"

# Formato adaptativo: Si no hay identidad, muestra título limpio sin espacios rotos.
_titulo_str = f"{_header_empresa} — " if _header_empresa and _header_empresa != "________________" else ""
_proyecto_str = f"Proyecto: {_header_proyecto} &nbsp;·&nbsp; " if _header_proyecto and _header_proyecto != "________________" else ""
...
<div style="background:linear-gradient(135deg,#0d1b2a 0%,#1e3a5f 60%,#2e6da4 100%);
 ...
 <div style="display:flex;align-items:center;gap:18px;">
  <div style="background:rgba(255,255,255,0.1); border-radius:12px; padding:12px; display:flex; align-items:center; justify-content:center;">
    <span style="font-size:42px; line-height:1;"></span>
  </div>
  <div>
   <h1 style="color:white;margin:0;font-size:2.2rem;font-weight:800;letter-spacing:-0.5px;">{_titulo_str}Calculadora de Materiales</h1>
   <p style="color:#90caf9;margin:4px 0 0;font-size:1rem;">{_proyecto_str}Multinorma &nbsp;·&nbsp; Terminología Regional &nbsp;·&nbsp; Precios en Tiempo Real</p>
  </div>
 </div>
</div>""", unsafe_allow_html=True)

# ── ONBOARDING — guía de primer uso (se oculta cuando ya tiene empresa y filas)
_has_identity = bool(st.session_state.get("empresa") and st.session_state.get("proyecto"))
_has_rows     = bool(st.session_state.get("kc_rows"))
if not _has_identity or not _has_rows:
  _step1_ok = "[Listo]" if _has_identity else "Paso 1:"
  _step2_ok = "[Listo]" if _has_rows    else "Paso 2:"
  _step3_ok = "Paso 3:"
  st.info(
    "Guia de inicio rapido — sigue estos 3 pasos:\n\n"
    f"{_step1_ok} **Configura tu empresa y proyecto** — abre 'Identidad del Proyecto' en el panel lateral izquierdo\n\n"
    f"{_step2_ok} **Calcula tus materiales** — selecciona cualquier tab (Concreto, Mamposteria, Varilla…) y presiona 'Agregar al resumen'\n\n"
    f"{_step3_ok} **Exporta tu memoria** — ve al tab Resumen y Exportar para descargar PDF, DOCX, DXF o Excel"
  )
...
with st.sidebar.expander(f" {_t('Identidad del Proyecto','Project Identity')}", expanded=False):
  _id_empresa = st.text_input(
    _t("Empresa / Firma","Company / Firm"),
    value=st.session_state.get("empresa",""),
    key="sid_empresa",
    placeholder="________________"
  )
  _id_proyecto = st.text_input(
    _t("Nombre del Proyecto","Project Name"),
    value=st.session_state.get("proyecto",""),
    key="sid_proyecto",
    placeholder="________________"
  )
  _id_ingeniero = st.text_input(
    _t("Ingeniero Responsable","Engineer in Charge"),
    value=st.session_state.get("ingeniero",""),
    key="sid_ingeniero",
    placeholder="________________"
  )
  _col_e, _col_r = st.columns(2)
  _id_elaboro = _col_e.text_input("Elaboró", value=st.session_state.get("dxf_elaboro",""), key="sid_elaboro", placeholder="___")
  _id_reviso = _col_r.text_input("Revisó", value=st.session_state.get("dxf_reviso",""), key="sid_reviso", placeholder="___")
  _id_aprobo = st.text_input("Aprobó", value=st.session_state.get("dxf_aprobo",""), key="sid_aprobo", placeholder="________________")
  if st.button(_t("Guardar identidad","Save identity"), key="btn_save_identity", use_container_width=True):
    st.session_state["empresa"]   = _id_empresa
    st.session_state["proyecto"]  = _id_proyecto
    st.session_state["ingeniero"]  = _id_ingeniero
    st.session_state["dxf_elaboro"] = _id_elaboro
    st.session_state["dxf_reviso"] = _id_reviso
    st.session_state["dxf_aprobo"] = _id_aprobo
    save_state()
    st.success(_t("Identidad guardada","Identity saved"))
    st.rerun()
  ...
  _logo_file = st.file_uploader(
    "Subir logo (PNG/JPG, recom. 400×200 px)",
    type=["png", "jpg", "jpeg"],
    key="sid_logo_upload",
    help="El logo se inserta en la portada del PDF y del DOCX exportados."
  )
  if _logo_file is not None:
    _logo_raw = _logo_file.read()
    if len(_logo_raw) > 0:
      st.session_state["logo_bytes"] = _logo_raw
      save_state()          # Persistir logo en disco
      st.success("Logo cargado y guardado")
  ...
  if st.session_state.get("logo_bytes"):
    try:
      st.image(
        ...
        width=160,
        caption="Logo activo — aparecerá en PDF y DOCX",
      )
    except Exception:
      st.warning("Logo cargado pero no se puede previsualizar. Prueba con PNG.")
    _col_logo_a, _col_logo_b = st.columns(2)
    with _col_logo_a:
      if st.button("Quitar logo", key="btn_remove_logo", use_container_width=True):
    ...
    with _col_logo_b:
      st.download_button(
        ...
        file_name="logo_empresa.png",
        mime="image/png",
        use_container_width=True,
        key="btn_dl_logo",
      )
  else:
    st.caption("Sin logo cargado — los documentos mostrarán solo texto.")
st.sidebar.markdown("---")
...
if st.sidebar.button(f" {_t('Consultar Precios en Vivo', 'Get Live Prices')}", use_container_width=True):
  with st.sidebar:
    with st.spinner(_t("Consultando precios...", "Fetching prices...")):
      live = get_live_prices(pais, ref)
      st.session_state.kc_precios = live
  _status = st.session_state.get("_price_status", "fallback")
  if _status == "live":
    st.sidebar.success(_t("Precios actualizados en vivo", "Live prices updated"))
  else:
    st.sidebar.warning(_t(
      " Sin datos en vivo — usando precios de referencia. Verifique con cotización.",
      " No live data — using reference prices. Verify with quotation."
    ))

# ── Indicador de estado de precios (siempre visible) ─────────────────────────
_ps_label = st.session_state.get("_price_status_label", "Precios de referencia (no consultados en vivo)")
_ps_status = st.session_state.get("_price_status", "fallback")
_ps_color  = "#1b5e20" if _ps_status == "live" else "#e65100"
_ps_icon   = "[+]" if _ps_status == "live" else "[!]"
st.sidebar.markdown(
  f'<div style="background:#1a1a1a;border-radius:6px;padding:6px 10px;margin:4px 0 8px;'
  f'font-size:10px;color:{_ps_color};line-height:1.4;">'
  f'{_ps_icon} {_ps_label}</div>',
  unsafe_allow_html=True
)
...
_footer_firma = st.session_state.get("empresa","") or "StructoPro"
st.sidebar.markdown(f'<div style="text-align:center;color:gray;font-size:10px;">© {datetime.datetime.now().year} {_footer_firma} — <br><i> Uso profesional exclusivo</i></div>',unsafe_allow_html=True)
...
with tabs[0]:
  ...
  with inp_col:
    ...
    if mode_conc == "Dosificación":
      ...
      # ── TABLA COMPARATIVA DE MEZCLAS ──────────────────────────────────
      with st.expander("Comparativa de todas las mezclas (costo por m³)", expanded=False):
        _precio_cem_t = float(p.get("cemento", 34000))
        _precio_are_t = float(p.get("arena",  80000))
        _precio_grav_t = float(p.get("grava",  90000))
        _peso_bolsa_t = float(R.get("peso_bolsa", 50))
        _mix_rows = []
        for _m in MIX_DESIGNS:
          _b  = _m["cem_kg"] / _peso_bolsa_t
          _c_c = _b * _precio_cem_t
          _c_a = _m["arena_m3"] * _precio_are_t
          _c_g = _m["grava_m3"] * _precio_grav_t
          _costo = round(_c_c + _c_a + _c_g, 0)
          _mix_rows.append({
            "Dos.": _m["dos"],
            "f'c kg/cm²": _m["fc_kgcm2"],
            "f'c MPa": _m["fc_mpa"],
            "Bolsas/m³": round(_b, 2),
            "Arena m³": _m["arena_m3"],
            "Grava m³": _m["grava_m3"],
            "Agua lt": _m["agua_lt"],
            f"Costo/m³ ({moneda})": _costo,
          })
        _mix_df_comp = pd.DataFrame(_mix_rows)
        _min_idx = _mix_df_comp[f"Costo/m³ ({moneda})"].idxmin()
        def _hl_min(row):
          if row.name == _min_idx:
            return ["background-color:#e8f5e9;font-weight:bold"] * len(row)
          return [""] * len(row)
        _mix_df_comp[f"Costo/m³ ({moneda})"] = _mix_df_comp[f"Costo/m³ ({moneda})"].apply(
          lambda x: f"{moneda} {x:,.0f}")
        st.dataframe(
          _mix_df_comp.style.apply(_hl_min, axis=1),
          use_container_width=True, height=390
        )
        st.caption("Verde = menor costo con precios actuales | Solo materiales (sin MO ni equipo)")
  ...
  with fig_col:
    ...
    fig3d.update_layout(scene=dict(aspectmode="data",
      ...
      bgcolor="#ffffff"),
      paper_bgcolor="#ffffff", margin=dict(l=0,r=0,t=0,b=0), height=260, font=dict(color="#1a1a1a"))
...
with tabs[1]:
  ...
  if modo_pared == " Block":
    ...
    with c1:
      ...
      m1, m2 = st.columns(2)
      m1.metric("Bloques", f"{und_blq:,} und")
      m2.metric("Cem. Pega", f"{bol_pega} {und_bol}")
      m3, m4 = st.columns(2)
      m3.metric("Arena", f"{arena_pega:.2f}m³")
      m4.metric("Agua", f"{agua_pega:.0f} lt")
      st.caption(f"Dosificación: {dos_pega}")
      
    with c2:
      st.markdown(f"** Acabados** ({moneda} {costo_acabados:,.0f})")
      a1, a2 = st.columns(2)
      a1.metric(f"Cem. {nom_rep}", f"{bol_rep} {und_bol}")
      a2.metric("Cem. Afinado", f"{bol_afin} {und_bol}")
      a3, a4 = st.columns(2)
      a3.metric("Arena", f"{arena_rep+arena_afin:.2f}m³")
      a4.metric("Agua", f"{agua_rep+agua_afin:.0f} lt")
 ...
 <div style="background:#1a1a0d;border:1px solid #6d6000;border-radius:8px;padding:6px 12px;min-width:140px;">
  ...
  # ========= LADRILLO =========
    ...
    with c1:
      st.markdown(f"** Muro Base** ({moneda} {costo_base:,.0f})")
      m1, m2 = st.columns(2)
      m1.metric("Ladrillos", f"{und_l:,} und")
      m2.metric("Cem. Pega", f"{bp} {und_bol}")
      m3, m4 = st.columns(2)
      m3.metric("Arena", f"{ap:.2f}m³")
      m4.metric("Agua", f"{agp:.0f} lt")
 ...
 <div style="background:#1a1a0d;border:1px solid #6d6000;border-radius:8px;padding:6px 12px;min-width:140px;">
  ...
  # ========= CICLÓPEO =========
    ...
    with c1:
      ...
      m1, m2 = st.columns(2)
      m1.metric("Piedra", f"{vol_piedra:.2f}m³")
      m2.metric("Cemento", f"{bol_c} {und_bol}")
      m3, m4 = st.columns(2)
      m3.metric("Arena", f"{arena_c_val:.2f}m³")
      m4.metric("Agua", f"{agua_c_val:.0f} lt")
...
"""
  ...
  with rc2:
    st.markdown(
      "<div style='text-align:center;font-size:1.8rem;'></div>"
      "<div style='text-align:center;font-weight:700;color:#ffcc80;margin-bottom:6px;'>Costos Concreto ({moneda})</div>",
      unsafe_allow_html=True)
    st.metric("Cemento", f"{_costo_cem:,.0f}")
    st.metric("Arena",  f"{_costo_are:,.0f}")
    st.metric("Grava",  f"{_costo_gra:,.0f}")
  ...
  with rc4:
    st.markdown(
      "<div style='text-align:center;font-size:1.8rem;'></div>"
      "<div style='text-align:center;font-weight:700;color:#ffcc80;margin-bottom:6px;'>Costos Acero ({moneda})</div>",
      unsafe_allow_html=True)
    st.metric("Longitudinal", f"{_costo_long:,.0f}")
    st.metric("Estribos",   f"{_costo_est:,.0f}")
    st.metric("Vol. concreto", f"{vol_col_neto*cant_col:.2f}m³")
    st.metric("COSTO TOTAL",  f"{costo_col:,.0f}")
...
with tabs[3]:
  ...
  with rv4:
    ...
    st.metric("Costo ({moneda})", f"{costo_v:,.0f}")
...
with tabs[4]:
  ...
  with rl4:
    ...
    st.metric("Costo ({moneda})", f"{costo_l:,.0f}")
  st.markdown(
    f'<div style="background:#0d2137;border-radius:8px;padding:8px 16px;">'
...
with tabs[5]:
  ...
  # ZAPATAS AISLADAS 
    ...
    with rz4: st.metric("Acero total",f"{kg_ace_z:.2f} kg"); st.metric("Costo ({moneda})",f"{costo_c:,.0f}")
  ...
  # LOSA CIMENTACIÓN POR % 
    ...
    with rp2: st.metric("Acero",f"{kg_ace_lc:.2f} kg"); st.metric("Dosif",mix_lc["dos"]); st.metric("Costo ({moneda})",f"{costo_c:,.0f}")
  ...
  # LOSA CIMENTACIÓN DIMENSIÓN 
    ...
    with rd4: st.metric("Total Acero",f"{kg_ace_ld:.2f}kg"); st.metric("Costo ({moneda})",f"{costo_c:,.0f}")
  ...
  # ZAPATA CORRIDA 
    ...
    with rzc4: st.metric("Total",f"{kg_ace_zc:.2f}kg"); st.metric("Costo ({moneda})",f"{costo_c:,.0f}")
  ...
  # ELECTROMALLA 
    ...
    with rem2: st.metric(" Electromalla",f"{n_piezas_em} piezas"); st.metric("Dosif.",mix_em["dos"]); st.metric("Volumen",f"{vol_em:.2f}m³"); st.metric("Costo ({moneda})",f"{costo_c:,.0f}")
  ...
  # SUELO CEMENTO 
    ...
    with rsc2: st.metric("Dosif.",dos_sc); st.metric("Volumen",f"{vol_suelo:.2f} m³"); st.metric("Costo cem",f"{costo_c:,.0f}")
...
with tabs[6]:
  ...
  if modo_m in [" Ciclópeo Trapecio"," Ciclópeo Trapecio+Pie",
    ...
    with rm2:
      ...
      st.metric("Costo est.",f"{costo_m:,.0f}")
  ...
  # ESTRUCTURAL 
    ...
    with re4: st.metric("Acero Total",f"{kg_ace_m:.2f}kg"); st.metric("Vol.Concreto",f"{vol_e:.2f}m³"); st.metric("Costo ({moneda})",f"{costo_m:,.0f}")
...
with tabs[14]:
  ...
  if sal:
    ...
    if fecha_salida >= fecha_ingreso:
      ...
      <h2>Liquidación Laboral — {pais}</h2>
      ...
      <p><small>Estimación referencial — verifique con la normativa local vigente.</small></p>
      </body></html>"""
...
with tabs[15]:
  ...
  _init_cfg("kc_cfg_adh_cer", pd.DataFrame([{"Material":"Pegante/Adhesivo (Cerámica)","REN(m²/bol)":2.0,"Agua(lt/bol)":9.45}]))
  _init_cfg("kc_cfg_boq_cer", pd.DataFrame([{"Material":"Boquilla/Fragua (Cerámica)","REN(m²/bol)":5.0,"Agua(lt/bol)":8.50}]))
  _init_cfg("kc_cfg_adh_pn", pd.DataFrame([{"Material":"Pegante/Adhesivo (Porcelanato)","REN(m²/bol)":2.0,"Agua(lt/bol)":9.45}]))
  _init_cfg("kc_cfg_boq_pn", pd.DataFrame([{"Material":"Boquilla/Fragua (Porcelanato)","REN(m²/bol)":5.0,"Agua(lt/bol)":8.50}]))
  ...
  _init_cfg("kc_cfg_costos", pd.DataFrame([
    ...
    {"Material": f"Cerámica Estándar (caja)",     "Costo": float(p.get("ceramica",12.0 if not _is_cop else 35000))},
    {"Material": f"Cerámica Premium (caja)",     "Costo": 14.0 if not _is_cop else 42000},
    {"Material": f"Porcelanato Estándar (caja)",    "Costo": 16.0 if not _is_cop else 55000},
    {"Material": f"Porcelanato Premium (caja)",    "Costo": 18.0 if not _is_cop else 68000},
    {"Material": f"Pegante/Adhesivo (Cerámica) (bolsa)",      "Costo": 8.0 if not _is_cop else 22000},
    {"Material": f"Boquilla/Fragua (Cerámica) (bolsa)",      "Costo": 6.0 if not _is_cop else 18000},
    {"Material": f"Pegante/Adhesivo (Porcelanato) (bolsa)",    "Costo": 10.0 if not _is_cop else 28000},
    {"Material": f"Boquilla/Fragua (Porcelanato) (bolsa)",    "Costo": 7.0 if not _is_cop else 22000},
  ]))
  ...
  with cfg_tabs[3]:
    ...
    st.info("Las conversiones: 1 barr ≈ 159 lt | 1 gal ≈ 3.785 lt")
    ...
    with cc2:
      if st.button("Consultar Precios en Vivo", key="cfg_live_prices", use_container_width=True):
        with st.spinner("Consultando precios..."):
          live = get_live_prices(pais, _ref_cos)
        _upd2=[k for k in live if abs(float(live.get(k,0))-float(_ref_cos.get(k,0) if isinstance(_ref_cos,dict) else 0))>0.5]
        st.session_state.kc_precios.update(live)
        st.session_state["_price_date"]=datetime.datetime.now().strftime("%d/%m/%Y %H:%M")
        if _upd2:
          st.session_state["_price_status"]="live"
          st.session_state["_price_status_label"]=f"Precios actualizados: {', '.join(_upd2[:4])}"
        else:
          st.session_state["_price_status"]="fallback"
          st.session_state["_price_status_label"]="Scraping sin datos — precios de referencia"
...
with tabs[7]:
  ...
  if "1A" in modo_t or "2A" in modo_t or "4A" in modo_t or "Área" in modo_t:
    ...
    with rt3: st.metric("Costo ({moneda})",f"{costo_t:,.0f}")
  ...
  # LÁMINAS (3 modos) 
    ...
    with rl3: st.metric("$/m",f"{precio_m_lam:.2f}"); st.metric("Costo ({moneda})",f"{costo_t:,.0f}")
...
with tabs[8]:
  ...
  if "Zócalo" not in modo_pi:
    ...
    with ri3:
      ...
      st.metric("Costo est.",f"{costo_pi:,.0f}")
  ...
  # ZÓCALO 
    ...
    with rz3:
      ...
      st.metric("Costo est.",f"{costo_z:,.0f}")
...
with tabs[9]:
  ...
  with rc3:
    ...
    st.metric("Costo total",f"{costo_cr:,.0f}")
...
with tabs[13]:
  ...
  st.download_button("Descargar Plantilla Excel",tmpl_buf,file_name="Plantilla_Materiales.xlsx",mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
with tabs[17]:
  ...
  st.markdown(f"#### Parámetros Base — {pais} (Valores en {moneda})")
  i1, i2, i3, i4, i5 = st.columns(5)
  i1.metric("Salario Mínimo (Mes)", f"{cfg_apu['smmlv_mes']:,.0f}")
  i2.metric("Auxilios/Adicionales", f"{cfg_apu.get('aux_trans', 0):,.0f}")
  i3.metric("Jornal c/prestaciones", f"{jd:,.0f}")
  ...
  try:
    ...
    st.metric(f"TOTAL PRESUPUESTO PRELIMINAR ESTIMADO", f"{_tot.sum():,.0f}")
  except: pass
...
with tabs[18]:
  ...
  if not st.session_state.kc_rows:
    ...
    with col_tbl:
      ...
      st.dataframe(df_d,use_container_width=True,height=350); st.metric("TOTAL ESTIMADO",f"{total_global:,.2f}")
    with col_chart:
      ...
      # MODO CLARO obligatorio para pantalla y para DOCX
    ...
    # ── Control de acceso — roles: admin | pro | free ────────────────────
    # admin: acceso total sin restricciones (modo desarrollo/revisión)
    # pro:   acceso a todos los exports
    # free:  solo Excel y CSV
    _rol = st.session_state.get("user_role", "free")
    _puede_exportar = _rol in ("admin", "pro")

    if _rol == "free":
      st.warning(
        "Modo Profesional requerido para exportar DOCX, DXF y PDF. "
        "Contacta al administrador para activar tu licencia."
      )
    elif _rol == "admin":
      st.info("Modo Administrador activo — todos los entregables habilitados.")

    _id_ok = bool(st.session_state.get("empresa") and st.session_state.get("proyecto"))
    if not _id_ok:
      st.warning("Completa Empresa y Proyecto en el panel lateral antes de exportar.")
    ...
    with ex1:
      ...
      st.download_button("Excel",excel_buf,
        file_name=f"Materiales_{(st.session_state.get('empresa','') or 'Proyecto').replace(' ','_')}_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",use_container_width=True)
    with ex2:
      csv_str=df_res.to_csv(index=False).encode("utf-8")
      st.download_button("CSV",csv_str,
        file_name=f"Materiales_{datetime.datetime.now().strftime('%Y%m%d')}.csv",
        mime="text/csv",use_container_width=True)
    with ex3:
      if not _puede_exportar:
        st.button("DOCX", disabled=True, help="Requiere licencia Pro o Admin", use_container_width=True)
      elif _DOCX_OK:
        docx_buf = build_docx_materiales(
          st.session_state.kc_rows, p, R,
          logo_bytes=st.session_state.get("logo_bytes")
        )
        if docx_buf:
          _proy_slug = (st.session_state.get("proyecto","Proyecto") or "Proyecto").replace(" ","_")
          st.download_button("DOCX",docx_buf,
            file_name=f"Memoria_{_proy_slug}_{datetime.datetime.now().strftime('%Y%m%d')}.docx",
            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            use_container_width=True, type="primary")
      else:
        st.button("DOCX", disabled=True, help="Instala: pip install python-docx", use_container_width=True)
    with ex4:
      if not _puede_exportar:
        st.button("DXF", disabled=True, help="Requiere licencia Pro o Admin", use_container_width=True)
      elif _DXF_OK:
        dxf_buf = build_dxf_planilla(st.session_state.kc_rows, p, R)
        if dxf_buf:
          _proy_slug = (st.session_state.get("proyecto","Proyecto") or "Proyecto").replace(" ","_")
          st.download_button("DXF",dxf_buf,
            file_name=f"Planilla_{_proy_slug}_{datetime.datetime.now().strftime('%Y%m%d')}.dxf",
            mime="application/octet-stream",
            use_container_width=True, type="primary")
      else:
        st.button("DXF", disabled=True, help="Instala: pip install ezdxf", use_container_width=True)
    with ex5:
      if not _puede_exportar:
        st.button("PDF", disabled=True, help="Requiere licencia Pro o Admin", use_container_width=True)
      elif _PDF_OK:
        _proy_slug_pdf = (st.session_state.get("proyecto","Proyecto") or "Proyecto").replace(" ","_")
        pdf_buf = build_pdf_materiales(
          st.session_state.kc_rows, p, R,
          logo_bytes=st.session_state.get("logo_bytes")
        )
        if pdf_buf:
          st.download_button("PDF", pdf_buf,
            file_name=f"Memoria_{_proy_slug_pdf}_{datetime.datetime.now().strftime('%Y%m%d')}.pdf",
            mime="application/pdf",
            use_container_width=True, type="primary")
      else:
        st.button("PDF", disabled=True,
          help="Instala: pip install reportlab",
          use_container_width=True)
    with ex6:
...
# ── ANÁLISIS DE SENSIBILIDAD DE PRECIOS ──────────────────────────────────────
    st.markdown("---")
    with st.expander("Análisis de Sensibilidad de Precios", expanded=False):
      _total_base = sum(float(r.get("cant",0))*float(r.get("precio",0))
               for r in st.session_state.get("kc_rows",[]))
      if _total_base > 0:
        _sc1, _sc2 = st.columns([2,3])
        with _sc1:
          _mat_s = st.selectbox("Material a variar:",
            ["Cemento","Acero / Varilla","Arena","Grava",
             "Bloque","Cerámica","Pintura","Todos"],
            key="sens_mat")
          _var_p = st.slider("Variación (%)",
            min_value=-50, max_value=100, value=15, step=5, key="sens_var")
        with _sc2:
          _wmap = {"Cemento":0.25,"Acero / Varilla":0.30,"Arena":0.08,
               "Grava":0.07,"Bloque":0.10,"Cerámica":0.08,
               "Pintura":0.05,"Todos":1.0}
          _w = _wmap.get(_mat_s, 0.20)
          _delta = _total_base * _w * (_var_p / 100.0)
          _nuevo = _total_base + _delta
          _color_s = "#c62828" if _delta > 0 else "#2e7d32"
          _sig = "+" if _delta > 0 else ""
          st.markdown(f"""<div style="background:#f5f5f5;border-radius:10px;padding:16px;margin-top:8px;">
<p style="margin:0;color:#555;font-size:0.9rem;">Si <b>{_mat_s}</b> varía <b>{_sig}{_var_p}%</b>:</p>
<h3 style="color:{_color_s};margin:6px 0;">{_sig}{moneda} {_delta:,.0f}</h3>
<p style="margin:0;color:#333;">Impacto total: <b>{_sig}{(_delta/_total_base*100):.1f}%</b></p>
<p style="margin:4px 0 0;font-size:0.85rem;color:#777;">
Base: {moneda} {_total_base:,.0f} → Ajustado: {moneda} {_nuevo:,.0f}</p>
</div>""", unsafe_allow_html=True)
      else:
        st.info("Agrega materiales para activar el análisis de sensibilidad.")

    # ── HUELLA DE CARBONO ─────────────────────────────────────────────────
    with st.expander("Huella de Carbono Estimada (CO2)", expanded=False):
      st.caption("Factores IPCC AR6 / ICE Database v3.0 — diferenciador único en LATAM")
      _co2_total = 0.0
      _co2_rows_list = []
      for _row in st.session_state.get("kc_rows", []):
        _el = str(_row.get("elemento","")).lower()
        _ca = float(_row.get("cant", 0))
        _f = 0.0
        if "cemento" in _el or "concreto" in _el:
          _f = _get_co2_factor("cemento_bolsa", pais)
        elif "acero" in _el or "varilla" in _el:
          _f = _get_co2_factor("acero_kg", pais)
        elif "arena" in _el:
          _f = _get_co2_factor("arena_m3", pais)
        elif "grava" in _el or "triturado" in _el:
          _f = _get_co2_factor("grava_m3", pais)
        elif "bloque" in _el:
          _f = _get_co2_factor("bloque_und", pais)
        elif "ladrillo" in _el:
          _f = _get_co2_factor("ladrillo_und", pais)
        elif "porcelanato" in _el:
          _f = _get_co2_factor("porcelanato_m2", pais)
        elif "cerám" in _el or "ceramica" in _el:
          _f = _get_co2_factor("ceramica_m2", pais)
        elif "pintura" in _el:
          _f = _get_co2_factor("pintura_galon", pais)
        elif "drywall" in _el or "yeso" in _el:
          _f = _get_co2_factor("drywall_m2", pais)
        elif "teja" in _el:
          _f = _get_co2_factor("teja_m2", pais)
        if _f > 0:
          _co2 = _ca * _f
          _co2_total += _co2
          _co2_rows_list.append({
            "Material": _row.get("elemento",""),
            "Cantidad": _ca,
            "Factor kg CO₂/ud": _f,
            "CO₂ (kg)": round(_co2, 1)
          })
      if _co2_rows_list:
        ...
        with _c1:
          st.dataframe(
            pd.DataFrame(_co2_rows_list).sort_values("CO₂ (kg)", ascending=False),
            use_container_width=True, height=260
          )
        with _c2:
          st.metric("CO₂ Total Estimado", f"{_co2_total:,.0f} kg")
          st.metric("Equiv. árboles/año", f"≈ {int(_co2_total/21):,}",
               help="1 árbol absorbe ≈ 21 kg CO₂/año (IPCC)")
          st.caption("Ningún competidor LATAM lo muestra actualmente.")
      else:
        st.info("Agrega materiales para calcular la huella de carbono.")
