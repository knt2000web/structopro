...
# ─── BANNER ESTÁNDAR DIAMANTE ───────────────────────────────────────────────
st.markdown("""<div style="width:100%;overflow:hidden;border-radius:14px;margin-bottom:18px;box-shadow:0 4px 32px #0008;"><svg viewBox="0 0 1100 220" xmlns="http://www.w3.org/2000/svg" style="width:100%;display:block;background:linear-gradient(135deg,#0a1128 0%,#1c2541 100%);"><g opacity="0.1" stroke="#38bdf8" stroke-width="0.5"><line x1="0" y1="55" x2="1100" y2="55"/><line x1="0" y1="110" x2="1100" y2="110"/><line x1="0" y1="165" x2="1100" y2="165"/><line x1="220" y1="0" x2="220" y2="220"/><line x1="440" y1="0" x2="440" y2="220"/><line x1="660" y1="0" x2="660" y2="220"/></g><rect x="0" y="0" width="1100" height="3" fill="#0ea5e9" opacity="0.9"/><rect x="0" y="217" width="1100" height="3" fill="#0ea5e9" opacity="0.7"/><g transform="translate(45,25)"><rect x="0" y="50" width="160" height="100" rx="3" fill="#1e293b" stroke="#0ea5e9" stroke-width="2" opacity="0.8"/><rect x="10" y="115" width="16" height="35" fill="#0ea5e9" opacity="0.7"/><rect x="32" y="105" width="16" height="45" fill="#38bdf8" opacity="0.7"/><rect x="54" y="95" width="16" height="55" fill="#7dd3fc" opacity="0.8"/><rect x="76" y="108" width="16" height="42" fill="#38bdf8" opacity="0.7"/><rect x="98" y="118" width="16" height="32" fill="#0ea5e9" opacity="0.7"/><line x1="0" y1="150" x2="160" y2="150" stroke="#0ea5e9" stroke-width="2" stroke-dasharray="4,3"/><rect x="130" y="70" width="28" height="28" rx="3" fill="#3a0000" stroke="#ef4444" stroke-width="2"/><text x="144" y="89" text-anchor="middle" font-family="monospace" font-size="9" fill="#fca5a5">C</text><text x="80" y="18" text-anchor="middle" font-family="sans-serif" font-size="9" font-weight="600" fill="#cbd5e1" letter-spacing="1">MENSULA + CORTANTE</text></g><g transform="translate(560,0)"><rect x="0" y="28" width="4" height="165" rx="2" fill="#0ea5e9"/><text x="18" y="66" font-family="Arial,sans-serif" font-size="28" font-weight="bold" fill="#ffffff">OTRAS ESTRUCTURAS</text><text x="18" y="94" font-family="Arial,sans-serif" font-size="14" font-weight="300" fill="#93c5fd" letter-spacing="2">CORBELS · CORTANTE · COLUMNAS · LOSAS</text><rect x="18" y="102" width="480" height="1" fill="#0ea5e9" opacity="0.5"/><rect x="18" y="115" width="130" height="22" rx="11" fill="#0c1a2e" stroke="#0ea5e9" stroke-width="1"/><text x="83" y="130" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" font-weight="bold" fill="#7dd3fc">CORTANTE A DIST. X</text><rect x="154" y="115" width="120" height="22" rx="11" fill="#3a0000" stroke="#ef4444" stroke-width="1"/><text x="214" y="130" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" font-weight="bold" fill="#fca5a5">MENSULAS ACI 318</text><rect x="282" y="115" width="110" height="22" rx="11" fill="#052e16" stroke="#10b981" stroke-width="1"/><text x="337" y="130" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" font-weight="bold" fill="#6ee7b7">COLUMNAS CORTAS</text><rect x="400" y="115" width="78" height="22" rx="11" fill="#291400" stroke="#f59e0b" stroke-width="1"/><text x="439" y="130" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" font-weight="bold" fill="#fcd34d">LOSAS 2D</text><text x="18" y="156" font-family="Arial,sans-serif" font-size="11" fill="#94a3b8">Modulo integrado de elementos no cubiertos por modulos especializados: ménsulas</text><text x="18" y="172" font-family="Arial,sans-serif" font-size="11" fill="#94a3b8">(Corbels) ACI 318 §16.5, cortante a dist. X en vigas, capacidad axial Pn,max</text><text x="18" y="188" font-family="Arial,sans-serif" font-size="11" fill="#94a3b8">de columnas cortas y momentos en losas bidireccionales (Metodo ACI Coeficientes).</text></g></svg></div>""", unsafe_allow_html=True)

with st.expander(" Guia Profesional — Ménsulas, Cortante, Columnas y Losas", expanded=False):
    st.markdown("""
    ### Metodología: Elementos Estructurales Complementarios (ACI 318 / NSR-10)
    Suite de herramientas para el diseño y verificacion de elementos que requieren tratamiento especial fuera del alcance de los modulos principales.

    ####  1. Cortante a una Distancia X del Apoyo (Vigas)
    - Calcula el cortante ultimo Vu en cualquier sección a lo largo de la viga para cargas distribuidas.
    - Determina el espaciamiento requerido de estribos: $s = A_v f_y d / V_s$, verificando los limites ACI de $d/2$ y $d/4$ (zonas criticas).
    - Genera visualizacion 3D interactiva de la viga con estribos posicionados y plano DXF ICONTEC.

    ####  2. Diseño de Ménsulas (Corbels) — ACI 318 §16.5
    - Aplica el modelo de biela-tirante (STM) y friccion de corte ($\mu = 1.4$ para concreto monolitico).
    - Calcula simultaneamente: acero por friccion de corte (Avf), por flexion (Af) y por traccion (An).
    - $A_s = \max(A_f + A_n,\; \frac{2}{3}A_{vf} + A_n,\; 0.04 f_c/f_y \cdot b_w d)$
    - Requiere estribos cerrados horizontales: $A_h = 0.5(A_s - A_n)$ distribuidos en los $2/3$ superiores de la mensula.

    ####  3. Capacidad Axial — Columnas Cortas (Pn,max)
    - Estima la carga axial ultima: $P_{n,max} = 0.80[0.85 f_c(A_g - A_{st}) + f_y A_{st}]$ (Estribos) o $0.85$ (Zunchos).
    - Verifica la cuantia $\rho \in [1\%, 8\%]$ segun NSR-10 C.10.3.

    ####  4. Losas Bidireccionales — Método de Coeficientes ACI
    - Para tableros con relacion $m = L_a/L_b \geq 0.5$, distribuye la carga en dos direcciones.
    - Selección del caso de borde (1 a 4) segun continuidad periférica y condiciones de apoyo.
    - Calcula momentos positivos y negativos de diseno por franja unitaria en cada direccion.
    """)
