    ...
    if L_pilote > prof_total:
            ...
            if _PLOTLY_AVAILABLE:
                ...
                for _, r_env in df.iterrows():
                    ...
                    if (r_env["z_bot"] - r_env["z_top"]) >= 0.5:
                        ...
                        fig_pf.add_annotation(x=1.5, y=(r_env["z_top"]+r_env["z_bot"])/2, text=texto_capa, showarrow=False,  align="center")
                ...
                fig_pf.add_annotation(x=1.0, y=L_pilote, text=" Base", showarrow=True, arrowhead=1, ax=40, ay=0)
        ...
        with col_opt2:
            ...
            if _PLOTLY_AVAILABLE and Pu_req > 0:
                ...
                fig_opt.update_layout(title="Curva de Optimización: Q_adm vs L_p", xaxis_title="Longitud Pilote L_p [m]", yaxis_title="Capacidad Admisible Q_adm [kN]", height=400, margin=dict(l=20, r=20, t=40, b=20))
...
with tab_grup:
    ...
    if _PLOTLY_AVAILABLE:
        ...
        if _PLOTLY_AVAILABLE:
            ...
            fig_g3d.update_layout(scene=dict(aspectmode='data', xaxis_title='X (cm)', yaxis_title='Y (cm)', zaxis_title='Z (cm)'),  height=550, margin=dict(l=0, r=0, b=0, t=30))
    ...
    with col_c3:
        ...
        if _PLOTLY_AVAILABLE:
            ...
            fig_p3d.update_layout(
                ...
                scene=dict(aspectmode='data', xaxis_title='X (cm)', yaxis_title='Y (cm)', zaxis_title='Z (cm)'),
                ...
                title=dict(text=f"Pilote {tipo_seccion} ({_D:.0f}cm) | Vista Modelo")
...
# --- P3: DIAGRAMA ESTRUCTURAL P-M ---
        ...
        try:
            ...
            fig_pm.update_layout(title="Diagrama P-M del Pilote Individual", xaxis_title="Momento (kN·m)", yaxis_title="Carga Axial (kN)")
        ...
        if not _DOCX_AVAILABLE:
            ...
            if st.button("Generar Súper Memoria", key="btn_docx"):
                ...
                try:
                    ...
                    doc.add_paragraph(f"- Capacidad por Fuste (Q_fuste): {Qs_total:.1f} kN\n"
                                      ...
                                      f"- Capacidad por Punta (Q_punta): {Qp:.1f} kN\n"
                                      ...
                                      f"- Capacidad Total Última (Q_total): {Qu:.1f} kN\n"
                                      ...
                                      f"- Factor de Seguridad Global (FS): {FS_global}\n"
                    ...
                    doc.add_paragraph(f"- Concreto f'c: {fc_pilote:.1f} MPa\n"
                                      ...
                                      f"- Acero Longitudinal: {n_barras_p} barras {barras_long} (Área = {n_barras_p * math.pi * (db_long_cm/2)**2:.2f} cm²)\n"
            ...
            if st.button("Generar Plano A1", key="btn_dxf"):
                ...
                try:
                    ...
                    for idx_s, (_, rst) in enumerate(df.iterrows()):
                        ...
                        _ritems.append((0.15, 17.0 - idx_s*0.5, 0.10, f"[{rst['z_top']}-{rst['z_bot']}m] {rst['Tipo']} | g={rst['γ (kN/m³)']:.1f} kN/m3"))
                    ...
                    #  ADICIÓN ALZADOS Y SECCIONES DE PILOTE AL DXF 
                    ...
                    msp.add_circle((cx_sc, cy_sc), D_pilote*100/2, dxfattribs={'layer':'PILOTES', 'color':3})
                    ...
                    rad_arm = (D_pilote*100/2) - (recub_pil) - (db_t_cm/2)
                    ...
                    if rad_arm > 0:
                        ...
                        msp.add_circle((cx_sc, cy_sc), rad_arm, dxfattribs={'layer':'REFUERZO', 'color':_color_acero_dxf(db_long_cm*10 if db_long_cm<5 else db_long_cm)})
                        ...
                        for bi in range(n_barras_p):
                            ...
                            bx = cx_sc + rad_long * math.cos(ang)
                            ...
                            by = cy_sc + rad_long * math.sin(ang)
                            ...
                            msp.add_circle((bx, by), db_long_cm/2, dxfattribs={'layer':'REFUERZO', 'color':_color_acero_dxf(db_long_cm*10 if db_long_cm<5 else db_long_cm)})
                    ...
                    msp.add_text("SECC. TRANSVERSAL PILOTE", dxfattribs={'height':0.4, 'layer':'TEXTOS'}).set_placement((cx_sc-(D_pilote*100/2)-1.0, cy_sc-(D_pilote*100/2)-2.5))
                    ...
                    Lp_dxf = min(L_pilote*100, 1500.0) 
                    ...
                    msp.add_line((ex_sc-D_pilote*100, ey_sc), (ex_sc+D_pilote*100, ey_sc), dxfattribs={'layer':'EJES_TERRENO', 'color':8, 'linetype':'DASHED'})
                    ...
                    msp.add_line((ex_sc-D_pilote*100/2, ey_sc), (ex_sc-D_pilote*100/2, ey_sc-Lp_dxf), dxfattribs={'layer':'PILOTES', 'color':3})
                    ...
                    msp.add_line((ex_sc-D_pilote*100/2, ey_sc-Lp_dxf), (ex_sc+D_pilote*100/2, ey_sc-Lp_dxf), dxfattribs={'layer':'PILOTES', 'color':3})
                    ...
                    if rad_arm > 0:
                        ...
                        msp.add_line((ex_sc-rad_long, ey_sc), (ex_sc-rad_long, ey_sc-Lp_dxf+10), dxfattribs={'layer':'REFUERZO', 'color':_color_acero_dxf(db_long_cm*10 if db_long_cm<5 else db_long_cm)})
                        ...
                        num_rings = int(Lp_dxf / s_trans_cm)
                        ...
                        for ri in range(min(num_rings, 100)):
                            ...
                            msp.add_line((ex_sc-rad_arm, ry), (ex_sc+rad_arm, ry), dxfattribs={'layer':'REFUERZO', 'color':_color_acero_dxf(db_long_cm*10 if db_long_cm<5 else db_long_cm)})
                    ...
                    msp.add_text("ALZADO LONG. PILOTE", dxfattribs={'height':0.4, 'layer':'TEXTOS'}).set_placement((ex_sc-1.0, ey_sc-Lp_dxf-3.0))
                    ...
                    st.download_button(" Descargar Plano.dxf", data=bio_dxf.getvalue().encode("utf-8"), file_name=f"Plano_Pilotes_{n_pilotes}u.dxf", mime="application/dxf")
                    ...
                    _config_pil = Configuration.defaults().with_changes(background_policy=BackgroundPolicy.WHITE)
                    Frontend(RenderContext(doc_dxf), MatplotlibBackend(_ax), config=_config_pil).draw_layout(msp, finalize=True)
